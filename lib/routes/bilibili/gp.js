...
module.exports = async (ctx) => {
    ...
    const items = list.map((item) => {
        ...
        return {
            title: iconv.decode(item.find("td").first().next().text(),'gbk'),
            description: iconv.decode(item.find("td").first().next().text(),'gbk'),
            link: `https://zhidao.baidu.com/${item.find("td").first().next().text()}`,
        };
    });
