...
module.exports = async (ctx) => {
    ...
    const items = list.map((item) => {
        ...
        return {
            title: iconv.decode(item.find("td").first().text()),
            description: iconv.decode(item.find("td").first().text()),
            link: `https://zhidao.baidu.com/${iconv.decode(item.find("td").first().text())}`,
        };
    });
