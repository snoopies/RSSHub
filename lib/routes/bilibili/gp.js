...
module.exports = async (ctx) => {
    ...
    const list = $("<tr class='LeftLiContainer'>").get();

    const items = list.map((item) => {
        item = $(item);

        return {
            title: item.find('td.lastNum').text(),
            description: item.find('td.lastNum').text(),
            link: `https://zhidao.baidu.com/${item.find('td.lastNum').text()}`,
        };
    });
